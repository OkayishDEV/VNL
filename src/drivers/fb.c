...
void fb_fill_gradient(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t c1, uint32_t c2)
{
    if (!s_fb_ok || w == 0 || h == 0) return;
    if (x >= s_fb.width || y >= s_fb.height) return;
    if (x + w > s_fb.width)  w = s_fb.width - x;
    ...
    int32_t r2 = (int32_t)((c2 >> 16) & 0xFF), g2 = (int32_t)((c2 >> 8) & 0xFF), b2 = (int32_t)(c2 & 0xFF);

    for (uint32_t row = 0; row < h; row++) {
        int32_t r = r1 + (r2 - r1) * (int32_t)row / (int32_t)h;
        int32_t g = g1 + (g2 - g1) * (int32_t)row / (int32_t)h;
        int32_t b = b1 + (b2 - b1) * (int32_t)row / (int32_t)h;
